...
| 2026-08-20 | Achado de auditoria: até aqui só existia `ResolveDispute` (endpoint manual do Admin) - o timeout automático de 7 dias descrito em "Prazo e timeout" nunca rodava, uma disputa sem decisão humana ficava `ABERTA` para sempre. `ResolveExpiredDisputes` (job diário) implementa o timeout. Não grava `Auditoria` (essa tabela exige `usuario_id` humano, `restrictOnDelete`); o motivo fica em `PaymentDispute.justificativa = TIMEOUT_AUTOMATICO`, mesmo padrão de `ApproveExpiredServices` para aprovação automática por hora expirada. |
